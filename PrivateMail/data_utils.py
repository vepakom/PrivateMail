...
def process_sop_data(data_path):
    ...
    for data_type, image_list in data_tuple.items():
        for index, line in tqdm(enumerate(open('{}/Ebay_{}.txt'.format(data_path, data_type), 'r', encoding='utf-8')),
                               desc= 'process uncropped data for cub dataset'):
            if index != 0:
...
def process_isc_data(data_path):
    ...
    for index, line in tqdm(enumerate(open('{}/Eval/list_eval_partition.txt'.format(data_path), 'r', encoding='utf-8'))):
        if index > 1:
    ...
    torch.save({'train': train_images, 'query': query_images, 'test': gallery_images},
               '{}/uncropped_data_dicts.pth'.format(data_path))

    
def process_fmnist_data(data_path):
    if not os.path.exists('{}/uncropped'.format(data_path)):
        os.mkdir('{}/uncropped'.format(data_path))
    train_images, test_images = {}, {}
    data_tuple = {'train': train_images, 'test': test_images}
...
def process_cifar_data(data_path):
    if not os.path.exists('{}/uncropped'.format(data_path)):
        os.mkdir('{}/uncropped'.format(data_path))
    train_images, test_images = {}, {}
    data_tuple = {'train': train_images, 'test': test_images}
    for data_type, image_list in data_tuple.items():
        for foldername in tqdm(os.listdir('{}/train'.format(data_path)), desc= 'process data for cifar dataset'):
            
            img = Image.open('{}/train/{}'.format(data_path, filename)).convert('RGB')
            save_name = '{}/uncropped/{}'.format(data_path, filename)
            img.save(save_name)
            if label in image_list:
                image_list[label].append(save_name)
            else:
...
if __name__ == '__main__':
    ...
    #process_isc_data('{}/isc'.format(opt.data_path))
     process_cifar_data('{}/cifar'.format(opt.data_path))
